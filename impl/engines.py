...
class FullQueryEngine(BasicQueryEngine):
    ...
    def getDiamondJournalsInAreasAndCategoriesWithQuartile(self, area_ids: set[str], category_ids: set[str], quartiles: set[str]):
        ...
        for handler in self.categoryQuery:
            ...
            with connect(path) as con:
                ...
                if area_list:
                    ...
                    conditions.append(f"Areas.area_name IN ({placeholders})")
                    params.extend(area_list)

                # Category + quartile pairs
                if cat_list and quart_list:
                    pair_conditions = []
                    for c in cat_list:
                        for q in quart_list:
                            pair_conditions.append("(Categories.category_name = ? AND JournalCategories.quartile = ?)")
                            params.extend([c, q])
        ...
        for row in df_apc["identifier"].astype(str).tolist():
